...
All Gmail API interactions are delegated to:
  - iobox.providers.google.auth      (authentication & profile)
  - iobox.providers.google._search   (search, batch metadata, sync history)
  - iobox.providers.google._retrieval (full content, batch fetch, labels, trash)
  - iobox.providers.google._sender   (compose, send, forward, drafts)
...
class GmailProvider(EmailProvider):
    ...
    def _to_email_data(self, raw: dict[str, Any]) -> EmailData:
        ...
        * Full-retrieval fields (``body``, ``content_type``, ``attachments``) may
        ...
        Args:
            raw: Dict returned by any function in ``_search`` or ``_retrieval``.
    ...
    def search_emails(self, query: EmailQuery) -> list[EmailData]:
        ...
        ``_search.search_emails`` via ``start_date`` / ``end_date``.
        ...
        # Always provide a start_date so _search doesn't fall back to the
