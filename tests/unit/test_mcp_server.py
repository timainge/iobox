...
class TestSearchGmail:
    def test_basic_search(self):
        with patch(f"{MODULE}.get_gmail_service") as mock_svc, \
             patch(f"{MODULE}.get_label_map", return_value={}) as mock_lm, \
            ...
            mock_search.assert_called_once_with(
                mock_svc.return_value, "from:test@example.com", 5, 3, None, None,
                label_map={}, include_spam_trash=False,
            )
    ...
    def test_search_with_dates(self):
        with patch(f"{MODULE}.get_gmail_service") as mock_svc, \
            ...
            mock_svc.return_value = MagicMock()
            result = search_gmail("subject:report", max_results=20, days=0,
...
class TestGetEmail:
    def test_get_email_html(self):
        with patch(f"{MODULE}.get_gmail_service") as mock_svc, \
             patch(f"{MODULE}.get_label_map", return_value={}), \
             patch(f"{MODULE}.get_email_content", return_value={"message_id": "m1", "subject": "Test"}) as mock_get:
            mock_svc.return_value = MagicMock()
            result = get_email("m1")
            assert result["subject"] == "Test"
            mock_get.assert_called_once_with(mock_svc.return_value, "m1", preferred_content_type="text/html", label_map={})
    ...
    def test_get_email_plain(self):
        with patch(f"{MODULE}.get_gmail_service") as mock_svc, \
            ...
            mock_get.assert_called_once_with(mock_svc.return_value, "m1", preferred_content_type="text/plain", label_map={})
...
class TestSaveEmail:
    ...
    def test_save_plain_text(self):
        with patch(f"{MODULE}.get_gmail_service") as mock_svc, \
            ...
            mock_get.assert_called_once_with(mock_svc.return_value, "m2", preferred_content_type="text/plain", label_map={})
    ...
    def test_send_with_cc_bcc(self):
        with patch(f"{MODULE}.get_gmail_service") as mock_svc, \
            ...
            mock_compose.assert_called_once_with(
                ...
                content_type="plain", attachments=None,
            )
...
class TestDrafts:
    ...
    def test_list_drafts(self):
        with patch(f"{MODULE}.get_gmail_service") as mock_svc, \
             patch(f"{MODULE}.list_drafts", return_value=[{"id": "d1", "subject": "Test"}]) as mock_list:
            mock_svc.return_value = MagicMock()
    ...
    def test_delete_draft(self):
        with patch(f"{MODULE}.get_gmail_service") as mock_svc, \
             patch(f"{MODULE}.delete_draft", return_value={"status": "deleted", "draft_id": "d1"}) as mock_del:
            mock_svc.return_value = MagicMock()
...
class TestCheckAuth:
    def test_check_auth_with_profile(self):
        with patch(f"{MODULE}.check_auth_status", return_value={"authenticated": True}), \
             patch(f"{MODULE}.get_gmail_service") as mock_svc, \
             patch(f"{MODULE}.get_gmail_profile", return_value={
    ...
    def test_check_auth_not_authenticated(self):
        with patch(f"{MODULE}.check_auth_status", return_value={"authenticated": False, "token_file_exists": False}), \
