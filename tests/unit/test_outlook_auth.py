...
class TestGetOutlookAccountBrowserFlow:
    def test_returns_account_when_already_authenticated(self, tmp_path):
        ...
        with patch.dict(sys.modules, {"O365": o365}):
            with patch("iobox.providers.outlook_auth.OUTLOOK_CLIENT_ID", "test-client-id"):
                with patch("iobox.providers.outlook_auth.OUTLOOK_TOKEN_DIR", token_dir):
    ...
    def test_triggers_browser_flow_when_not_authenticated(self, tmp_path):
        ...
        with patch.dict(sys.modules, {"O365": o365}):
            with patch("iobox.providers.outlook_auth.OUTLOOK_CLIENT_ID", "test-client-id"):
                with patch("iobox.providers.outlook_auth.OUTLOOK_TENANT_ID", "common"):
                    with patch("iobox.providers.outlook_auth.OUTLOOK_TOKEN_DIR", token_dir):
                        import iobox.providers.outlook_auth as mod
                        result = mod.get_outlook_account()
    ...
    def test_raises_value_error_when_client_id_missing(self):
        ...
        with patch.dict(sys.modules, {"O365": o365}):
            with patch("iobox.providers.outlook_auth.OUTLOOK_CLIENT_ID", ""):
                import iobox.providers.outlook_auth as mod
...
class TestTokenFilePath:
    ...
    def test_account_constructed_with_credentials_and_tenant(self, tmp_path):
        ...
        with patch.dict(sys.modules, {"O365": o365}):
            with patch("iobox.providers.outlook_auth.OUTLOOK_CLIENT_ID", "my-client-id"):
                with patch("iobox.providers.outlook_auth.OUTLOOK_CLIENT_SECRET", "my-secret"):
                    with patch("iobox.providers.outlook_auth.OUTLOOK_TENANT_ID", "my-tenant"):
                        with patch("iobox.providers.outlook_auth.OUTLOOK_TOKEN_DIR", token_dir):
...
class TestCheckOutlookAuthStatus:
    ...
    def test_returns_not_authenticated_when_token_file_missing(self, tmp_path):
        ...
        with patch("iobox.providers.outlook_auth.OUTLOOK_CLIENT_ID", "test-id"):
            with patch("iobox.providers.outlook_auth.OUTLOOK_TOKEN_DIR", non_existent_dir):
                import iobox.providers.outlook_auth as mod
                status = mod.check_outlook_auth_status()
    ...
    def test_returns_authenticated_when_token_valid(self, tmp_path):
        ...
        with patch.dict(sys.modules, {"O365": o365}):
            with patch("iobox.providers.outlook_auth.OUTLOOK_CLIENT_ID", "test-id"):
                with patch("iobox.providers.outlook_auth.OUTLOOK_TOKEN_DIR", str(token_dir)):
                    import iobox.providers.outlook_auth as mod
                    status = mod.check_outlook_auth_status()
    ...
    def test_tenant_id_in_status(self):
        with patch("iobox.providers.outlook_auth.OUTLOOK_CLIENT_ID", ""):
            with patch("iobox.providers.outlook_auth.OUTLOOK_TENANT_ID", "mytenant"):
                import iobox.providers.outlook_auth as mod
                status = mod.check_outlook_auth_status()
