...
class TestAuthentication:
    @patch("iobox.providers.gmail._auth.get_gmail_service")
...
class TestSearchAndRead:
    ...
    def test_search_emails(self, mock_search, mock_label_map, provider):
        ...
        mock_search.return_value = [
            {"message_id": "m1", "from": "a@b.com", "subject": "Hi"}
        ]
...
class TestSendForwardDrafts:
    @patch("iobox.providers.gmail._sender.send_message")
    @patch("iobox.providers.gmail._sender.compose_message")
    ...
    @patch("iobox.providers.gmail._sender.forward_email")
    ...
    @patch("iobox.providers.gmail._sender.create_draft")
    @patch("iobox.providers.gmail._sender.compose_message")
    def test_create_draft(self, mock_compose, mock_create, provider):
        ...
        result = provider.create_draft(
            to="a@b.com", subject="Draft", body="body", cc="c@d.com"
        )
        mock_compose.assert_called_once_with(
            to="a@b.com",
            subject="Draft",
    ...
    @patch("iobox.providers.gmail._sender.list_drafts")
    ...
    @patch("iobox.providers.gmail._sender.send_draft")
    ...
    @patch("iobox.providers.gmail._sender.delete_draft")
...
class TestSystemOperations:
    @patch("iobox.providers.gmail._retrieval.modify_message_labels")
    def test_mark_read(self, mock_modify, provider):
        provider.mark_read("m1", read=True)
        mock_modify.assert_called_once_with(
            provider._service, "m1", remove_labels=["UNREAD"]
        )
    ...
    def test_mark_unread(self, mock_modify, provider):
        ...
        mock_modify.assert_called_once_with(
            provider._service, "m1", add_labels=["UNREAD"]
        )
    ...
    def test_set_star_true(self, mock_modify, provider):
        ...
        mock_modify.assert_called_once_with(
            provider._service, "m1", add_labels=["STARRED"]
        )
    ...
    def test_set_star_false(self, mock_modify, provider):
        ...
        mock_modify.assert_called_once_with(
            provider._service, "m1", remove_labels=["STARRED"]
        )
    ...
    def test_archive(self, mock_modify, provider):
        provider.archive("m1")
        mock_modify.assert_called_once_with(
            provider._service, "m1", remove_labels=["INBOX"]
        )

    @patch("iobox.providers.gmail._retrieval.trash_message")
    def test_trash(self, mock_trash, provider):
        provider.trash("m1")
        mock_trash.assert_called_once_with(provider._service, "m1")

    @patch("iobox.providers.gmail._retrieval.untrash_message")
...
class TestTagOperations:
    @patch("iobox.providers.gmail._retrieval.modify_message_labels")
    @patch("iobox.providers.gmail._retrieval.resolve_label_name")
    def test_add_tag(self, mock_resolve, mock_modify, provider):
        ...
        mock_modify.assert_called_once_with(
            provider._service, "m1", add_labels=["Label_123"]
        )
    ...
    def test_remove_tag(self, mock_resolve, mock_modify, provider):
        ...
        mock_modify.assert_called_once_with(
            provider._service, "m1", remove_labels=["Label_456"]
        )
